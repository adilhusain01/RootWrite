      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert Solidity developer specializing in Rootstock smart contracts. Your task is to generate secure, optimized smart contract code based on user requirements. Focus on Rootstock compatibility and gas efficiency. Include helpful comments in the code.",
        },
        {
          role: "user",
          content: prompt,
...
function generatePrompt(
  ...
  basePrompt +=
    "1. If using OpenZeppelin v5.0 or newer, Ownable requires a constructor parameter: `constructor() Ownable(msg.sender) {...}`\n";
  basePrompt +=
    "2. If using OpenZeppelin v4.x, Ownable doesn't need parameters: `constructor() {...}`\n";
  basePrompt +=
    "3. Always check that import statements match the version you're using\n";
  basePrompt +=
    "4. For maximum compatibility, specify the exact OpenZeppelin version in import statements\n\n";

  switch (template) {
    case "erc20":
      basePrompt +=
        "Create an ERC-20 token contract compatible with Rootstock. Use OpenZeppelin's ERC20 implementation for security. ";
      basePrompt +=
        'IMPORTANT: If using OpenZeppelin v5.0+, constructor needs proper initialization like `constructor() ERC20("Name", "Symbol") Ownable(msg.sender) {...}`. ';
      basePrompt +=
        "Follow the latest OpenZeppelin documentation at https://docs.openzeppelin.com/contracts/5.x/erc20 for best practices.";
      break;
    case "erc721":
      basePrompt +=
        "Create an ERC-721 NFT contract compatible with Rootstock. Use OpenZeppelin's ERC721 implementation for security. ";
      basePrompt +=
        'IMPORTANT: If using OpenZeppelin v5.0+, constructor needs proper initialization like `constructor() ERC721("Name", "Symbol") Ownable(msg.sender) {...}`. ';
      basePrompt +=
        "Follow the latest OpenZeppelin documentation at https://docs.openzeppelin.com/contracts/5.x/erc721 for best practices.";
      break;
    case "multisig":
      basePrompt +=
        "Create a multi-signature wallet contract compatible with Rootstock. Ensure it has proper security measures. ";
      basePrompt +=
        "Reference OpenZeppelin's governance contracts at https://docs.openzeppelin.com/contracts/5.x/governance for implementation ideas.";
      break;
    case "voting":
      basePrompt +=
        "Create a voting contract compatible with Rootstock. Include access control and transparent vote counting. ";
      basePrompt +=
        "Follow the voting standards at https://docs.openzeppelin.com/contracts/5.x/governance for best practices.";
      break;
    default:
      basePrompt +=
        "Use Solidity version 0.8.20 or later, which is compatible with Rootstock. ";
      basePrompt +=
        "Follow modern Solidity patterns from the Solidity documentation at https://docs.soliditylang.org/en/v0.8.20/. ";
  }
...
function parseResponse(responseContent: string): {
  ...
  if (responseContent.includes("---")) {
    ...
    let match;
    const codeBlocks = [];
    const tempContent = responseContent;
...
function checkRootstockCompatibility(contractCode: string): string {
  ...
  if (
    ...
    contractCode.includes("pragma solidity ^0.4")
  ) {
    warnings.push(
      "Consider using Solidity version 0.8.x for compatibility with Rootstock"
    );
  }
  ...
  if (
    contractCode.includes("uint8") ||
    contractCode.includes("uint16") ||
    contractCode.includes("uint32") ||
    contractCode.includes("uint64") ||
    contractCode.includes("uint128")
  ) {
    warnings.push(
      "Consider using uint256 instead of smaller uint types for better gas efficiency"
    );
  }
